...
// Store a user avatar. Squares it down to 256px when ffmpeg is available so
// profile pictures never ship a 10 MB original to every visitor.
export async function storeAvatar(file) {
  if (classify(file.type) !== "image") throw new Error("avatar must be an image");
  mkdirSync(join(UPLOAD_ROOT, "avatars"), { recursive: true });

  const id = randomUUID();
  const rel = `avatars/${id}.${EXT[file.type] || "bin"}`;
  await Bun.write(join(UPLOAD_ROOT, rel), await file.arrayBuffer());
  if (!hasFfmpeg) return rel;

  const relSmall = `avatars/${id}.256.jpg`;
  const absSmall = join(UPLOAD_ROOT, relSmall);
  const proc = Bun.spawnSync([
    ffmpegPath, "-loglevel", "error", "-i", join(UPLOAD_ROOT, rel),
    "-vf", "scale=256:256:force_original_aspect_ratio=increase,crop=256:256",
    "-frames:v", "1", "-y", absSmall,
  ]);
  if (proc.exitCode !== 0 || !existsSync(absSmall)) return rel;
  rmSync(join(UPLOAD_ROOT, rel), { force: true });
  return relSmall;
}

// Remove stored files (original + preview) for a deleted row.
export function deleteFiles(...rels) {
  for (const rel of rels) {
    if (!rel) continue;
    const abs = join(UPLOAD_ROOT, rel);
    if (!abs.startsWith(UPLOAD_ROOT)) continue; // never escape the upload root
